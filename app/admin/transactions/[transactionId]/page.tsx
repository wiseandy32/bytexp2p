    ...
    if (action === "approve") {
      try {
        ...
        if (userDocSnap.exists()) {
          ...
          const emailApiEndpoint = isDeposit
        ...
        toast.error(
          `An error occurred while approving the ${isDeposit ? "deposit" : "withdrawal"}.`,
        );
      }
    } else {
      // 'reject'
      try {
        ...
        toast.success(
          `${isDeposit ? "Deposit" : "Withdrawal"} rejected successfully!`,
        );
        router.push("/admin/transactions");
      } catch (error) {
        console.error(`Error rejecting transaction ${transactionId}:`, error);
        toast.error(
          `An error occurred while rejecting the ${isDeposit ? "deposit" : "withdrawal"}.`,
        );
      }
    }
  };
          ...
          <div className="flex gap-4">
            <AlertDialog>
              <AlertDialogTrigger asChild>
                <Button disabled={transaction.status !== "awaiting_approval"}>
                  Approve
                </Button>
              </AlertDialogTrigger>
              <AlertDialogContent>
                <AlertDialogHeader>
                  <AlertDialogTitle>
                    Are you sure you want to approve this {transaction.type}?
                  </AlertDialogTitle>
                  <AlertDialogDescription>
                    This action will{" "}
                    {transaction.type === "deposit" ? "credit" : "debit"} the
                    user's account with {transaction.amount}{" "}
                    {transaction.token.shortName}. This action cannot be undone.
                  </AlertDialogDescription>
                </AlertDialogHeader>
                <AlertDialogFooter>
            ...
            <AlertDialog>
              <AlertDialogTrigger asChild>
                <Button
                  variant="destructive"
                  disabled={transaction.status !== "awaiting_approval"}
                >
                  Reject
                </Button>
              </AlertDialogTrigger>
              <AlertDialogContent>
                <AlertDialogHeader>
                  <AlertDialogTitle>
                    Are you sure you want to reject this {transaction.type}?
                  </AlertDialogTitle>
                  <AlertDialogDescription>
                    This will mark the transaction as rejected. This action
                    cannot be undone.
                  </AlertDialogDescription>
                </AlertDialogHeader>
                <AlertDialogFooter>
