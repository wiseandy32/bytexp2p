      ...
      <div className="bg-gray-800 rounded-lg shadow-xl m-4 max-w-md w-full">
        ...
        <div className="p-3">
          <form onSubmit={handleSubmit}>
            <input type="hidden" name="_token" value="LezWTWg3oR0pi2dex0oDeDpZ4G9NGtLoecM0oo33" />
            <div className="mb-4">
              <label className="block text-gray-400 text-sm mb-2" htmlFor="currency_id">
              ...
              >
                {tokens.map(token => (
                  <option key={token.value} value={token.value}>{token.name}</option>
                ))}
              </select>
            </div>
            <div className="mb-4">
            ...
            <p className="text-gray-500 text-xs">
              Note: Ensure that the address is correct and on the same network. Contact <a href="mailto:support@peershieldex.com" className="text-blue-400 hover:underline">support@peershieldex.com</a> for guidance.
            </p>
            <div className="mt-4 text-center">
              <button type="submit" className="bg-blue-600 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded-md text-xs">
                Proceed
              </button>
            </div>
          </form>
